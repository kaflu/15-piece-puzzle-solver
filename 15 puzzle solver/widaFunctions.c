    ...
    *expandedNodes = 0;
    ...
    while(result == NULL)
    {
        ...
        *tempBound = INFINITY;
        ...
        result = iterativeDeepeningAStar(Node, bound, tempBound, generatedNodes, expandedNodes);
...
/** IDA* Algorithm */
node *iterativeDeepeningAStar(node *Node, int bound, int *tempBound, long long *generatedNodes, long long *expandedNodes)
{
    /** Iterator */
    int i;
    ...
    for(i = 0; i < MAX_POSSIBLE_MOVES; i++)
    {
        ...
        *generatedNodes = *generatedNodes + 1;
        ...
        /** Update the state and store it in newState 
          * n'.s = f(a, n.s) 
          */
        tempNode->state = actionState(moveList[i], Node->state, newState);
        ...
        /** Update the threshold 
          *  n'.f(n) = n'.g(n) + h(n'.s)
          */
        tempNode->threshold = tempNode->cost + WEIGHT * heuristicUnweightedValue(tempNode->state);
        ...
        else 
        {
            ...
            *expandedNodes = *expandedNodes + 1;
            ...
            /** Recursively repeat */
            result = iterativeDeepeningAStar(tempNode, bound, tempBound, generatedNodes, expandedNodes);
...
int *actionState(int movePosition, int state[SIZE], int newState[SIZE])
{
    /** Iterator */
    int i;
